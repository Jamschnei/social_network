...
def home():	
	...
	#notes
	currentUserId = getUserId('sessionId',getCookie('_aok_'))
	allNotes      = fetchAll('sender,note,noteId,closed,favorited','notesData','toUserId',currentUserId)
...
def profile(username):
	...
	if info:
		bottle.response.set_cookie('_unm_', username, expires = 60 * 30, path='/')
...
def addNote():
	...
	noteId    = ''.join( [random.choice( string.letters + string.digits ) for i in range( 26 )] )
	toUserId  = getUserId('username',getCookie('_unm_'))
	closed    = False
	favorited = False
	...
	notesInfo = (sender,toUserId,noteId,note,reply,fromUserId,fromEmail,closed,favorited)
	try:
		database.execute('''INSERT INTO notesData(sender,toUserId,noteId,note,reply,fromUserId,fromEmail,closed,favorited) 
							VALUES(?,?,?,?,?,?,?,?,?)''',notesInfo)
		connection.commit()
		return "Message sent"
	except:
		return "There was some problem, please try again."#@bottle.redirect(/database-error) TODO
		
#note options
@bottle.post('/notes/favoritetoggle/')
def addNoteToFavorites():
	noteId 		 = getFormValue('noteId')
	auth         = getUserId('sessionId',getCookie('_aok_'))
	noteStatus   = fetchOne('favorited','notesData','noteId',noteId)
	
	if noteStatus:
		update('notesData','favorited',True,'noteId',noteId)
		return "favorite" #return text that says add to favorites i.e note is not in favoite list
	else:	
		update('notesData','favorited',False,'noteId',noteId)
		return "remove"
	
@bottle.route('/notes/close/:noteId')
def closeNote(noteId):
	auth         = getUserId('sessionId',getCookie('_aok_'))
	update('notesData','closed',True,'noteId',noteId)
	return "done"
